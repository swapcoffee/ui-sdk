<template>
  ...
  >
    <div :class="{ show_expert: getExpertMode }" class="settings__content custom-scroll">
      <div>
      ...
      <!-- <div class="settings__item-mode">
                <div class="settings__group">
                    <div class="settings__info">
                        <ProtectIcon :customClass="'settings__icon'" />
                ...
                <switch-toggle :class="{ active_switch: getMevProtectionValue }" @click="switchMevProtection"/>
            </div> -->
      <div class="settings__item-mode">
        <div class="settings__group">
          <div class="settings__info">
            <img
                ...
                src="../../assets/dex/expert-mode.svg"
            />
            <p class="settings__name">
      ...
      <div v-show="getExpertMode" class="settings__show-more">
        ...
        <div class="settings__item" v-if="getRouteName === 'Dex'">
          <p class="settings__text">
          ...
          <p class="settings__description">
            ...
            <span v-if="!isV5">
                            ...
                            <strong class="description-color">{{
                                $t("dexSettings.parallel.colorText")
                              }}</strong>
                        </span>
            <span
                class="info-wrapper"
          ...
          <label v-if="isV5" class="settings__label" for="">
            <input
                v-model="maxSplits"
                autocomplete="off"
                class="settings__input"
          ...
          <ul class="settings__block-list">
            ...
            >
              <div class="block-item__left">
                ...
                />
                <p class="block-item__title">{{ source?.name }}</p>
              </div>
              <switch-toggle
                  :class="{ active_switch: isActiveSource(source.id) }"
                  @click="changeLiquiditySources(source.id)"
              />
            </li>
          </ul>
        </div>
...
import MevSettings from '@/components/dex/mev/MevSettings.vue';
import {DEXES, DEXES_BY_ID} from "@/utils/dexes.ts";
...
export default {
  ...
  data() {
    return {
      ...
      isV5: false,
      showTooltip: false,
      liquiditySources: DEXES.map(it => ({...it, enabled: true}))
    };
  },
  computed: {
    dexStore() {
    ...
    getMevProtectionValue() {
      return this.dexSettingsStore.GET_MEV_PROTECTION_VALUE
    },
  ...
  methods: {
    ...
    switchMevProtection() {
      const updatedMevProtection = !this.getMevProtectionValue
      this.dexSettingsStore.DEX_MEV_PROTECTION(updatedMevProtection)
      this.saveToStorage("mevProtection", updatedMevProtection)
    },
    ...
    changeLiquiditySources(value, notSave) {
      ...
      const enabledSources = this.liquiditySources.filter((e) => e.enabled).map((e) => e.id);
    ...
    async checkStorageSettings() {
      try {
        ...
        if (settings.hasOwnProperty("mevProtection")) {
          this.dexSettingsStore.DEX_MEV_PROTECTION(settings.mevProtection)
        }
    ...
    isActiveSource(source) {
      return this.liquiditySources.some((s) => s.id === source && s.enabled);
    },
  },
  watch: {
