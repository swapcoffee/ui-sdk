<template>
  <div class="settings-background" @click.self="$emit('closeSettings')">
    <div v-if="!hideSettings" class="settings">
      ...
      <div class="settings__chart-mode">
        <div class="settings__group">
          <div class="settings__info">
            <img src="@/assets/dex/chart-setting.svg" alt="chart icon" class="settings__icon" />
            <p class="settings__name">
              {{ $t('dexGlobal.chart.title') }}
            </p>
          </div>
          <p class="settings__description">
            {{ $t('dexGlobal.chart.description') }}
          </p>
        </div>
        <switch-toggle :class="{ active_switch: getChartVisibleValue }" @click="switchChartVisibilitySetting" />
      </div>
      ...
      <div class="settings__chart-mode">
        <div class="settings__group">
          <div class="settings__info">
            <p class="settings__name">
              {{ $t('dexGlobal.chart-profit.title') }}
            </p>
          </div>
          <p class="settings__description">
            {{ $t('dexGlobal.chart-profit.caption') }}
          </p>
        </div>
        <switch-toggle :class="{ active_switch: getChartTradingProfitValue }" @click="switchChartTradingProfitSetting" />
      </div>

      <div class="settings__chart-mode">
        <div class="settings__group">
          <p class="settings__name">
            {{ $t('dexGlobal.dev.title') }}
          </p>
          <p class="settings__description">
            {{ $t('dexGlobal.dev.description') }}
          </p>
        </div>
        <switch-toggle :class="{ active_switch: getDevModeIsEnabled }" @click="switchDevModeSetting" />
      </div>
      <div class="theme">
        ...
        <div class="theme__switch-block">
          ...
          >
            ...
            >
              <path
                v-if="theme === 'system'"
                d="M10 13.5H6C5.86739 13.5 5.74021 13.5527 5.64645 13.6464C5.55268 13.7402 5.5 13.8674 5.5 14C5.5 14.1326 5.55268 14.2598 5.64645 14.3536C5.74021 14.4473 5.86739 14.5 6 14.5H10C10.1326 14.5 10.2598 14.4473 10.3536 14.3536C10.4473 14.2598 10.5 14.1326 10.5 14C10.5 13.8674 10.4473 13.7402 10.3536 13.6464C10.2598 13.5527 10.1326 13.5 10 13.5Z"
                fill="white"
              />
              <path
                v-if="theme === 'system'"
                d="M13 2.5H3C2.17157 2.5 1.5 3.17157 1.5 4V11C1.5 11.8284 2.17157 12.5 3 12.5H13C13.8284 12.5 14.5 11.8284 14.5 11V4C14.5 3.17157 13.8284 2.5 13 2.5Z"
                fill="white"
              />
              <path
                v-else
                d="M13 2.5H3C2.60218 2.5 2.22064 2.65804 1.93934 2.93934C1.65804 3.22064 1.5 3.60218 1.5 4V11C1.5 11.3978 1.65804 11.7794 1.93934 12.0607C2.22064 12.342 2.60218 12.5 3 12.5H13C13.3978 12.5 13.7794 12.342 14.0607 12.0607C14.342 11.7794 14.5 11.3978 14.5 11V4C14.5 3.60218 14.342 3.22064 14.0607 2.93934C13.7794 2.65804 13.3978 2.5 13 2.5ZM13.5 11C13.5 11.1326 13.4473 11.2598 13.3536 11.3536C13.2598 11.4473 13.1326 11.5 13 11.5H3C2.86739 11.5 2.74021 11.4473 2.64645 11.3536C2.55268 11.2598 2.5 11.1326 2.5 11V4C2.5 3.86739 2.55268 3.74021 2.64645 3.64645C2.74021 3.55268 2.86739 3.5 3 3.5H13C13.1326 3.5 13.2598 3.55268 13.3536 3.64645C13.4473 3.74021 13.5 3.86739 13.5 4V11ZM10.5 14C10.5 14.1326 10.4473 14.2598 10.3536 14.3536C10.2598 14.4473 10.1326 14.5 10 14.5H6C5.86739 14.5 5.74021 14.4473 5.64645 14.3536C5.55268 14.2598 5.5 14.1326 5.5 14C5.5 13.8674 5.55268 13.7402 5.64645 13.6464C5.74021 13.5527 5.86739 13.5 6 13.5H10C10.1326 13.5 10.2598 13.5527 10.3536 13.6464C10.4473 13.7402 10.5 13.8674 10.5 14Z"
                fill="white"
              />
            </svg>
          </button>
      ...
      <div class="settings__chart-mode cursor-pointer" @click="toggleSettings(() => { chartTypeOpen = true })">
        <div class="settings__group">
          <p class="settings__name">
            {{ $t('dexGlobal.chart-provider.title') }}
          </p>
          <p class="settings__description">
            {{ getChartTypeValue }}
          </p>
        ...
        >
          <path
            d="M12.7729 9.39792L7.14792 15.0229C7.09565 15.0752 7.03361 15.1166 6.96533 15.1449C6.89704 15.1732 6.82386 15.1878 6.74995 15.1878C6.67604 15.1878 6.60285 15.1732 6.53457 15.1449C6.46628 15.1166 6.40424 15.0752 6.35198 15.0229C6.29972 14.9707 6.25826 14.9086 6.22998 14.8403C6.20169 14.772 6.18713 14.6989 6.18713 14.6249C6.18713 14.551 6.20169 14.4779 6.22998 14.4096C6.25826 14.3413 6.29972 14.2792 6.35198 14.227L11.5797 8.99995L6.35198 3.77292C6.24643 3.66737 6.18713 3.52421 6.18713 3.37495C6.18713 3.22568 6.24643 3.08253 6.35198 2.97698C6.45753 2.87143 6.60068 2.81213 6.74995 2.81213C6.89921 2.81213 7.04237 2.87143 7.14792 2.97698L12.7729 8.60198C12.8252 8.65422 12.8667 8.71626 12.895 8.78454C12.9233 8.85283 12.9379 8.92603 12.9379 8.99995C12.9379 9.07387 12.9233 9.14706 12.895 9.21535C12.8667 9.28364 12.8252 9.34567 12.7729 9.39792Z"
            fill="white"
          />
        </svg>
      </div>
      <div class="lang cursor-pointer" @click="toggleSettings(() => { langMode = true })">
        <div class="lang__info">
          <p class="settings__name">
            {{ $t('dexGlobal.lang.name') }}
          </p>
          <p class="settings__description">
            {{ $t('dexGlobal.lang.text') }}
          </p>
        </div>
        <svg
          class="lang__arrow-icon"
    ...
    <div v-if="langMode" class="settings">
      <div class="settings__title">
        <button class="settings__close-btn" @click="toggleSettings(() => { langMode = false })">
        ...
        <h2 class="settings__heading">
          {{ $t('dexGlobal.lang.caption') }}
        </h2>
        <div class="hidden-block" />
      </div>
      <div class="settings-list">
        <button
          class="settings-list__btn"
          :class="{ active_btn: lang === 'en' }"
          @click="changeLang('en')"
        >
          <div class="btn-body">
            <strong class="btn-title">English</strong>
            <p class="btn-subtitle">English</p>
          </div>
          <DoneIcon v-if="lang === 'en'" class="done-icon" />
        </button>
        <button
          class="settings-list__btn"
          :class="{ active_btn: lang === 'ru' }"
          @click="changeLang('ru')"
        >
          <div class="btn-body">
            <strong class="btn-title">Russian</strong>
            <p class="btn-subtitle">Русский</p>
          </div>
          <DoneIcon v-if="lang === 'ru'" class="done-icon" />
        </button>
        <button
          class="settings-list__btn"
          :class="{ active_btn: lang === 'ua' }"
          @click="changeLang('ua')"
        >
          <div class="btn-body">
            <strong class="btn-title">Ukrainian</strong>
            <p class="btn-subtitle">Українська</p>
          </div>
          <DoneIcon v-if="lang === 'ua'" class="done-icon" />
        </button>
        <button
          class="settings-list__btn"
          :class="{ active_btn: lang === 'es' }"
          @click="changeLang('es')"
        >
          <div class="btn-body">
            <strong class="btn-title">Spanish</strong>
            <p class="btn-subtitle">Español</p>
          </div>
          <DoneIcon v-if="lang === 'es'" class="done-icon" />
        </button>
        <button
          class="settings-list__btn"
          :class="{ active_btn: lang === 'zh' }"
          @click="changeLang('zh')"
        >
          <div class="btn-body">
            <strong class="btn-title">Chinese</strong>
            <p class="btn-subtitle">中文</p>
          </div>
          <DoneIcon v-if="lang === 'zh'" class="done-icon" />
        </button>
        <button
          class="settings-list__btn"
          :class="{ active_btn: lang === 'fr' }"
          @click="changeLang('fr')"
        >
          <div class="btn-body">
            <strong class="btn-title">French</strong>
            <p class="btn-subtitle">Français</p>
          </div>
          <DoneIcon v-if="lang === 'fr'" class="done-icon" />
        </button>
        <button
          class="settings-list__btn"
          :class="{ active_btn: lang === 'fa' }"
          @click="changeLang('fa')"
        >
          <div class="btn-body">
            <strong class="btn-title">Fārsi</strong>
            <p class="btn-subtitle">فارسی</p>
          </div>
          <DoneIcon v-if="lang === 'fa'" class="done-icon" />
        </button>
      </div>
    </div>
    <div v-if="chartTypeOpen" class="settings">
      <div class="settings__title">
        <button class="settings__close-btn" @click="toggleSettings(() => { chartTypeOpen = false })">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
          ...
          >
            <path
              d="M6.96933 12.5307L14.4693 20.0307C14.539 20.1004 14.6217 20.1556 14.7128 20.1933C14.8038 20.2311 14.9014 20.2505 14.9999 20.2505C15.0985 20.2505 15.1961 20.2311 15.2871 20.1933C15.3782 20.1556 15.4609 20.1004 15.5306 20.0307C15.6003 19.961 15.6555 19.8783 15.6932 19.7872C15.731 19.6962 15.7504 19.5986 15.7504 19.5001C15.7504 19.4015 15.731 19.3039 15.6932 19.2129C15.6555 19.1218 15.6003 19.0391 15.5306 18.9694L8.56026 12.0001L15.5306 5.03068C15.6713 4.88995 15.7504 4.69907 15.7504 4.50005C15.7504 4.30103 15.6713 4.11016 15.5306 3.96943C15.3898 3.8287 15.199 3.74963 14.9999 3.74963C14.8009 3.74963 14.6101 3.8287 14.4693 3.96943L6.96933 11.4694C6.89959 11.5391 6.84427 11.6218 6.80653 11.7128C6.76879 11.8039 6.74936 11.9015 6.74936 12.0001C6.74936 12.0986 6.76879 12.1962 6.80653 12.2873C6.84427 12.3783 6.89959 12.461 6.96933 12.5307Z"
              fill="white"
            />
          </svg>
        </button>
        <h2 class="settings__heading">
          {{ $t('dexGlobal.chart-provider.caption') }}
        </h2>
        <div class="hidden-block" />
      </div>
      <div class="settings-list">
        <button
          class="settings-list__btn"
          :class="{ active_btn: getChartTypeValue === 'TradingView' }"
          @click="changeChartType('TradingView')"
        >
          <div class="btn-body">
            <strong class="btn-title">DYOR</strong>
            <p class="btn-subtitle">{{ $t('dexGlobal.chart-provider.providers-description[0]') }}</p>
          </div>
          <DoneIcon v-if="getChartTypeValue === 'TradingView'" class="done-icon" />
        </button>
        <button
          class="settings-list__btn"
          :class="{ active_btn: getChartTypeValue === 'Lightweight' }"
          @click="changeChartType('Lightweight')"
        >
          <div class="btn-body">
            <strong class="btn-title">DYOR Light</strong>
            <p class="btn-subtitle">{{ $t('dexGlobal.chart-provider.providers-description[1]') }}</p>
          </div>
          <DoneIcon v-if="getChartTypeValue === 'Lightweight'" class="done-icon" />
        </button>
        <button
          class="settings-list__btn"
          :class="{ active_btn: getChartTypeValue === 'GeckoTerminal' }"
          @click="changeChartType('GeckoTerminal')"
        >
          <div class="btn-body">
            <strong class="btn-title">GeckoTerminal</strong>
            <p class="btn-subtitle">{{ $t('dexGlobal.chart-provider.providers-description[2]') }}</p>
          </div>
          <DoneIcon v-if="getChartTypeValue === 'GeckoTerminal'" class="done-icon" />
        </button>
        <button
          class="settings-list__btn"
          :class="{ active_btn: getChartTypeValue === 'Dexscreener' }"
          @click="changeChartType('Dexscreener')"
        >
          <div class="btn-body">
            <strong class="btn-title">Dexscreener</strong>
            <p class="btn-subtitle">{{ $t('dexGlobal.chart-provider.providers-description[3]') }}</p>
          </div>
          <DoneIcon v-if="getChartTypeValue === 'Dexscreener'" class="done-icon" />
        </button>
      </div>
    </div>
  </div>
...
import dexGlobalSettingsMixin from '@/mixins/dexGlobalSettingsMixin.js';
import DoneIcon from "@/assets/dex/icons/Done.vue";

export default {
  name: 'DexGlobalSettings',
  components: { SwitchToggle, DoneIcon },
  data() {
    return {
      hideSettings: false,
    };
  ...
  // mounted() {
  // this.checkUserSettings(JSON.parse(localStorage.getItem('globalSettings')))
  // },
  watch: {
    GET_USER_SETTINGS: {
      handler() {
        this.checkUserSettings(this.GET_USER_SETTINGS?.globalSettings);
      },
    },
  },
  created() {
...
.theme-coffee .active_btn {
  background: var(--iface-accent-color);
}

.theme-light .active_btn .btn-title {
  color: var(--main-white);
}

.theme-light .active_btn .btn-subtitle {
  color: var(--main-white);
}

.theme-coffee .active_btn .btn-title {
  color: var(--main-white);
}

.theme-coffee .active_btn .btn-subtitle {
  color: var(--main-white);
}
