<template>
  <div class="settings-background" @click.self="$emit('closeSettings')">
    <div class="settings">
      <div class="settings__title">
        <h2 class="settings__heading">
          {{ $t('dexSettings.caption') }}
        </h2>
        <button class="settings__close-btn" @click="$emit('closeSettings')">
          <svg
...
export default {
  ...
  methods: {
    ...
    changeSlippageInput(event) {
      ...
      this.dexStore.DEX_SLIPPAGE(this.slippage);
      this.saveToStorage('slippage', this.slippage);
    },
    blurSlippageInput() {
    ...
    changePriceImpactInput(event) {
      ...
      this.dexStore.DEX_PRICE_IMPACT(this.priceImpact);
