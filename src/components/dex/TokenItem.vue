<template>
	<li class="tokens-popup__item item">
		<div class="item__group">
			<div class="item__image-block">
				<img :src="item?.image" alt="Token logo in TON blockchain" class="item__image" v-if="item?.image !== 'null'">
...
<!--						<img src="@/assets/dex/token-link.svg" alt="link icon" class="item__link-icon">-->
				...
				<div class="item__group"
					v-if="status !== 'unlisted' && item?.imported !== true"
				>
					<svg class="item__price-arrow up-price" v-if="item?.price_change_24h > 0" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
						<path d="M11.779 8.91735C11.7459 8.9973 11.6898 9.06564 11.6179 9.11373C11.546 9.16182 11.4614 9.18749 11.3749 9.18751H2.62486C2.53828 9.18758 2.45362 9.16195 2.38161 9.11389C2.3096 9.06582 2.25348 8.99746 2.22033 8.91748C2.18719 8.83749 2.17853 8.74947 2.19544 8.66456C2.21235 8.57965 2.25407 8.50166 2.31532 8.44048L6.69032 4.06548C6.73096 4.0248 6.77921 3.99253 6.83232 3.97051C6.88543 3.9485 6.94236 3.93716 6.99986 3.93716C7.05735 3.93716 7.11428 3.9485 7.16739 3.97051C7.2205 3.99253 7.26876 4.0248 7.30939 4.06548L11.6844 8.44048C11.7456 8.5017 11.7872 8.57968 11.804 8.66456C11.8209 8.74945 11.8122 8.83742 11.779 8.91735Z" fill="#55FF85"/>
					</svg>
					<svg class="item__price-arrow down-price" v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
						<path d="M11.779 5.08265C11.7459 5.0027 11.6898 4.93436 11.6179 4.88627C11.546 4.83818 11.4614 4.81251 11.3749 4.81249H2.62486C2.53828 4.81242 2.45362 4.83805 2.38161 4.88611C2.3096 4.93418 2.25348 5.00254 2.22033 5.08252C2.18719 5.16251 2.17853 5.25053 2.19544 5.33544C2.21235 5.42035 2.25407 5.49834 2.31532 5.55952L6.69032 9.93452C6.73096 9.9752 6.77921 10.0075 6.83232 10.0295C6.88543 10.0515 6.94236 10.0628 6.99986 10.0628C7.05735 10.0628 7.11428 10.0515 7.16739 10.0295C7.2205 10.0075 7.26876 9.9752 7.30939 9.93452L11.6844 5.55952C11.7456 5.4983 11.7872 5.42032 11.804 5.33544C11.8209 5.25055 11.8122 5.16258 11.779 5.08265Z" fill="#FF5449"/>
					</svg>
					...
					>
						...
						<svg class="item__icon" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
...
export default {
  ...
  computed: {
    ...
    getTokenPrice(): string {
      const value = this.item?.price_usd || 0;
      if (value >= 1000) {
        return `$${this.filterNumber(value.toFixed(2))}`;
    ...
    getTokenTvl(): number | string {
      const tvlInTon = (this.item?.tvl || 0) / this.tonPrice;
      const reduceNum = this.filterBalance(tvlInTon);
    ...
    getTokenHolders(): number | string {
      const reduceNum = this.filterBalance(this.item?.holders_count || 0);
      if (typeof reduceNum === "number") {
        return reduceNum.toFixed(1);
      } else if (typeof reduceNum === "object") {
        return `${reduceNum.result}${reduceNum.unit}`;
    ...
    getTokenBalance(): string {
      return this.item?.balance > 0 ? this.item?.balance.toFixed(2) : "0";
    },
    GET_PINNED_TOKENS() {
