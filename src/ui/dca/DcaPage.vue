...
export default {
  ...
  computed: {
    ...
    getLimitSuborders() {
      if (!this.setDcaTransactionInfo.maxSuborders || this.setDcaTransactionInfo.maxSuborders < 2) {
        return 2;
      }
      return this.setDcaTransactionInfo.maxSuborders;
    },
    getDcaDeposit() {
      const amount = this.setDcaTransactionInfo.firstAmount;
      const symbol = this.setDcaTransactionInfo.firstToken?.symbol || '';
      return `${this.prettyNumber(amount, 2)} ${symbol}`;
    },
    getEachOrderSize() {
      const amount = this.setDcaTransactionInfo.firstAmount;
      const perOrder = amount / this.getLimitSuborders;
      const symbol = this.setDcaTransactionInfo.firstToken?.symbol || '';
      return `${this.prettyNumber(perOrder, 2)} ${symbol}`;
    },
    getEstimatedEndDate() {
      const delay = this.getLimitSuborders > 1
          ? this.setDcaTransactionInfo.intervalSec * (this.getLimitSuborders - 1)
          : 0;
    ...
    getPriceRange() {
      if (!this.dcaStore.GET_DCA_ENABLE_RANGE) return '';
    ...
    dcaDetails() {
      ...
      if (this.dcaStore.GET_DCA_ENABLE_RANGE &&
          this.dcaStore.GET_MIN_RANGE !== null &&
          this.dcaStore.GET_MAX_RANGE !== null) {
        details.push({
